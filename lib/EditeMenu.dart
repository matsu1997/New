...
class EditeMenu extends StatefulWidget {
  @override
  State<EditeMenu> createState() => _EditeMenuState();
}

class _EditeMenuState extends State<EditeMenu> {
  var item = [];
   var Id = "";
  @override
  void initState() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text("メニュー編集", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black),),
          centerTitle: true,
          iconTheme: IconThemeData(color: Colors.black),
          actions: <Widget>[
            IconButton(
                onPressed: () {  Navigator.of(context).push(
                  MaterialPageRoute(builder: (context) => EditeMenuAdd()),).then((value) {_loadData();});},
                icon:Icon(Icons.add))
          ],
        ),
        body: Container(
...
class _EditeMenuAddState extends State<EditeMenuAdd> {
  ...
  Future<void> addFilePath(String userId, String localPath) async {
    FirebaseFirestore.instance.collection('メニュー').doc(Id).set({
      "text": text,
      "subText":subText,
      "値段":limit,
      "時間":time,
