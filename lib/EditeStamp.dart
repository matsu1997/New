...
class EditeStamp extends StatefulWidget {
  @override
  State<EditeStamp> createState() => _EditeStampState();
}

class _EditeStampState extends State<EditeStamp> {
  var item = [];
  var Id = "";
  @override
  void initState() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text("スタンプ編集", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black),),
          centerTitle: true,
          iconTheme: IconThemeData(color: Colors.black),
          actions: <Widget>[
            IconButton(
                onPressed: () {  Navigator.of(context).push(
                  MaterialPageRoute(builder: (context) => EditeStampAdd()),).then((value) {_loadData();});},
                icon:Icon(Icons.add))
          ],
        ),
        body: Container(
                    ...
                    return GestureDetector(
                        onTap: () {
                          showDialog(context: context, builder: (context) =>
                              AlertDialog(
                                title: Text("削除しますか？",textAlign: TextAlign.center,style: TextStyle(fontWeight: FontWeight.bold,),),
                                actions: <Widget>[
                                  Column(children: [
                                    Container(
                              ...
                              Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                          margin: EdgeInsets.only(top: 5, left: 10, right: 10),
                                          child: Text(item[index]["ポイント"].toString()+ "ポイント",
                                            style: TextStyle(
                                                color: Colors.grey,
                                                fontWeight: FontWeight.bold,
                                                fontSize: 10),
  ...
  void delete (){
    setState(() {
      FirebaseFirestore.instance
          .collection("スタンプ").doc(Id).delete();
      _loadData();});Navigator.of(context).pop();}
}


class EditeStampAdd extends StatefulWidget {
  @override
  _EditeStampAddState createState() => _EditeStampAddState();
}

class _EditeStampAddState extends State<EditeStampAdd> {
  ...
  var pointCon = TextEditingController();
  String email = '';
  ...
  Future<void> addFilePath(String userId, String localPath) async {
    FirebaseFirestore.instance.collection('スタンプ').doc(Id).set({
      "text": text,
      "subText":subText,
      "有効期限":limit,
      'imageId': imgPathUse,
            ...
            child: Container(
                child: Column(children: <Widget>[
                  ...
                  Container(
                    ...
                    child:TextFormField(
                      controller: textCon,
                      decoration: InputDecoration(labelText:"獲得チケット名"),
                  ...
                  Container(
                    ...
                    child:TextFormField(
                      ...
                      maxLines: null,
                      controller: pointCon,
                      decoration: InputDecoration(labelText:"必要ポイント数"),
                      onChanged: (String value3) {
                        setState(() {point = int.parse(value3); });
                      },),),
                  Container(
                    margin: EdgeInsets.only(top:20,bottom: 20),width: 100,
                    child: ElevatedButton(
