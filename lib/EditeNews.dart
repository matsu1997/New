...
class EditeNews extends StatefulWidget {
  @override
  State<EditeNews> createState() => _EditeNewsState();
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text("最新情報編集", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black),),
          centerTitle: true,
          iconTheme: IconThemeData(color: Colors.black),
          actions: <Widget>[
            IconButton(
                onPressed: () {  Navigator.of(context).push(
                  MaterialPageRoute(builder: (context) => EditeNewsAdd()),).then((value) {_loadData();});},
                icon:Icon(Icons.add))
          ],
        ),
        body: Container(
...
class EditeNewsAdd extends StatefulWidget {
  @override
  _EditeNewsAddState createState() => _EditeNewsAddState();
}

class _EditeNewsAddState extends State<EditeNewsAdd> {
  ...
  Future<void> addFilePath(String userId, String localPath) async {
    FirebaseFirestore.instance.collection('最新情報').doc(userId).set({
      "title":title,
      "text": text,
      "date":date,
      'ImageId': imgPathUse,
