...
class EditeTicket extends StatefulWidget {
  @override
  State<EditeTicket> createState() => _EditeTicketState();
}

class _EditeTicketState extends State<EditeTicket> {
  var item = [];
  var Id = "";
  @override
  void initState() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text("チケット編集", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black),),
          centerTitle: true,
          iconTheme: IconThemeData(color: Colors.black),
          actions: <Widget>[
            IconButton(
                onPressed: () {  Navigator.of(context).push(
                  MaterialPageRoute(builder: (context) => EditeTicketAdd()),).then((value) {_loadData();});},
                icon:Icon(Icons.add))
          ],
        ),
        body: Container(
              ...
              return GestureDetector(
                        onTap: () {
                          showDialog(context: context, builder: (context) =>
                              AlertDialog(
                                title: Text("チケットを削除しますか？",textAlign: TextAlign.center,style: TextStyle(fontWeight: FontWeight.bold,),),
                                actions: <Widget>[
                                  Column(children: [
                                    Container(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: ElevatedButton(child: Text("送る"), onPressed: () {Navigator.of(context).push(
                                          MaterialPageRoute(builder: (context) => EditeTicketV2(item[index]["text"],item[index]["subText"],item[index]["imageId"],item[index]["有効期限"],item[index]["Id"],)),).then((value) {_loadData();});},),)
                                    ],
                                  )),],)),));},),),
              Container()
            ])));
  ...
  void _loadData()  {
    item = [];
    FirebaseFirestore.instance.collection('チケット').get().then((QuerySnapshot snapshot) {
  ...
  void delete (){
    setState(() {
      FirebaseFirestore.instance
          .collection("チケット").doc(Id).delete();
      _loadData();});Navigator.of(context).pop();}
}


class EditeTicketAdd extends StatefulWidget {
  @override
  _EditeTicketAddState createState() => _EditeTicketAddState();
}

class _EditeTicketAddState extends State<EditeTicketAdd> {
  ...
  Future<void> addFilePath(String userId, String localPath) async {
    FirebaseFirestore.instance.collection('チケット').doc(Id).set({
      "text": text,
      "subText":subText,
      "有効期限":limit,
      'imageId': imgPathUse,
            ...
            child: Container(
                child: Column(children: <Widget>[
                  ...
                  Container(
                    ...
                    child:TextFormField(
                      controller: textCon,
                      decoration: InputDecoration(labelText:"チケット名"),
...
class EditeTicketV2 extends StatefulWidget {
  EditeTicketV2(this.text,this.subText,this.imageId,this.limit,this.Id);
  String text;
  String subText;
  String Id;
  int limit;
  String imageId;
  @override
  State<EditeTicketV2> createState() => _EditeTicketV2State();
}
class _EditeTicketV2State extends State<EditeTicketV2> {
  var item = [];
  var item1 = [];
  String infoText = '';
  String email = '';
                                    ...
                                    child:Row(children: [
                                      Container(width: 110,margin: EdgeInsets.only(bottom: 5,left:5,right: 5),child: ElevatedButton(
                                        ...
                                        onPressed: () {
                                          ...
                                          await FirebaseFirestore.instance.collection('users').doc(item[index]["uid"]).collection("チケット").doc(Id).set({
                                            "text":widget.text,
  ...
  void send (){
    for (var co = 0; co < item1.length  ; co++) {
      var Id = randomString(20);
      FirebaseFirestore.instance.collection('users').doc(item1[co]).collection("チケット").doc(Id).set({
        "text": widget.text,
        "subText": widget.subText,
        "有効期限": widget.limit,
        "imageId": widget.imageId,
